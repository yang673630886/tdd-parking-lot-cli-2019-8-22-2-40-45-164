...
class ParkingBoyFacts {
	...
	@Test
	void should_park_second_lot_when_park_if_first_lot_is_empty() {
		ParkingLot firstParkingLot = new ParkingLot(0);
		ParkingLot secondParkingLot = new ParkingLot(2);
		ParkingLot[] parkingLots = new ParkingLot[2];
		parkingLots[0] = firstParkingLot;
		parkingLots[1] = secondParkingLot;
		Car car = new Car();
		ParkingBoy parkingBoy = new ParkingBoy( parkingLots);
		ParkingTicket ticket = parkingBoy.park(car);
		
		assertNotNull(ticket);
		assertEquals(1, parkingLots[1].getAvailableParkingPosition());
